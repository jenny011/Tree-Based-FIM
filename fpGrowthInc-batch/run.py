...
if __name__ == '__main__':
	...
	parser.add_argument('--batch', '-c', help='batch number', required=False)
	...
	for num in range(int(args.expnum)):
		...
		for i in range(1, 51, 5):
			...
			base = totalDB[:base_size]

			dbItems = getDBItems(base)
			fpTree = buildFPTree(base, dbItems)

			f_perf.write(str(i) + ",")
			# incremental
			times = []
			for j in range(base_size, db_size, update_interval):
				if db_size - j < update_interval:
					break

				incDB = totalDB[j : min(db_size, j + update_interval)]

				start = time()
				base += incDB
				newDBItems = getDBItems(base)
				unsorted = {}
				...
				if temp_keys == headers:
					...
					fpTree = buildFPTree(base, newDBItems)
					results = mineAll(fpTree, '')
				end = time()
