...
if __name__ == "__main__":
	...
	if d:
		...
		for i in range(0, len(datasets)-1):
			lookup_paths = [os.path.join(os. getcwd(), "results", f"lookup_{datasets[i]}.csv")]
			insert_paths = [os.path.join(os. getcwd(), "results", f"insert_{datasets[i]}.csv")]
			...
			plt.errorbar(insert_x, insert_means, yerr=insert_errs, color=colors[i], linestyle="dashdot", label=datasets[i] + '_insert')

		plt.title(f"Python dictionary performance - datasets - random")
